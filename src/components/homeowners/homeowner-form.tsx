...
export function HomeownerForm({ initialData, onSubmit, isEditing = false }: HomeownerFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8 max-w-4xl mx-auto px-2 sm:px-0">
      {/* SECTION 1: Principal Homeowner Information */}
      <div className="rounded-3xl border border-slate-200/80 dark:border-[#1e2f4d] bg-white dark:bg-[#0e192d] p-4 sm:p-6 md:p-8 shadow-subtle space-y-4 sm:space-y-6">
          ...
          <div className="relative">
            {photoPath && isValidImageUrl(photoPath) ? (
              <img 
                src={photoPath} 
                alt="Profile" 
                className="h-20 w-20 sm:h-24 sm:w-24 rounded-full object-cover border-4 border-slate-200 dark:border-slate-700" 
                onError={(e) => {
                  e.currentTarget.style.display = 'none';
                }}
              />
            ) : (
              <div className="h-20 w-20 sm:h-24 sm:w-24 rounded-full bg-slate-100 dark:bg-slate-800 border-4 border-slate-200 dark:border-slate-700 flex items-center justify-center">
