...
export function ManagePermissionsModal({
  ...
  const [permissions, setPermissions] = useState<UserPermissions>(
    targetUser?.permissions || DEFAULT_PERMISSIONS_BY_ROLE.user
  );
  ...
  const grantAll = () => {
    setPermissions(DEFAULT_PERMISSIONS_BY_ROLE.super_admin);
  };
          ...
          <div className="p-4 rounded-2xl bg-emerald-50/80 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 text-emerald-950 dark:text-emerald-200 flex items-start gap-3 shadow-xs">
            ...
            <div className="text-xs space-y-1">
              ...
              <p className="text-emerald-800 dark:text-emerald-300 leading-relaxed">
                As the HOA President (Super Admin), this account inherently commands all permissions. Permissions cannot be revoked for the Super Admin to preserve system continuity.
              </p>
            </div>
          </div>
        ) : (
