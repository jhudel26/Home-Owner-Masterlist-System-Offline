...
export function BackupRestoreModal({ isOpen, onClose }: BackupRestoreModalProps) {
  ...
  const handleApplyRestore = async () => {
    ...
    const result = await Swal.fire({
      ...
      html: `Are you sure you want to restore data from <b>${restoreFileName || "the selected backup file"}</b>?<br/><br/><span class="text-xs text-amber-600 dark:text-amber-400 font-semibold">⚠️ Warning: Existing homeowner records that match will be updated or overwritten.</span>`,
      icon: "warning",
