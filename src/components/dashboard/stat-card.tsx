...
export const StatCard = React.memo(function StatCard({
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-slate-200/90 dark:border-white/10 bg-white dark:bg-[#0e192d]/90 backdrop-blur-xl p-4 sm:p-5 shadow-subtle hover:shadow-card hover:border-emerald-500/30 dark:hover:border-emerald-500/30 hover:-translate-y-0.5 transition-all duration-200">
      {/* Top Header */}
      <div className="flex items-center justify-between">
        <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
          {title}
        </span>
        <div className={cn("p-2 sm:p-2.5 rounded-xl border transition-transform duration-200 group-hover:scale-105", iconVariants[variant])}>
      ...
      <div className="mt-2 sm:mt-3">
        <div className="flex items-baseline justify-between gap-2">
          <span className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white font-sans tracking-tight font-mono">
            {value}
          </span>
          {badgeText && (
            <span className="text-[10px] sm:text-[11px] font-semibold px-1.5 sm:px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700/80">
              {badgeText}
            </span>
          )}
        </div>
        ...
        {progressPercent !== undefined && (
          <div className="mt-2 sm:mt-3 w-full bg-slate-100 dark:bg-slate-800/80 rounded-full h-1.5 overflow-hidden">
            <div
        ...
        {subtitle && (
          <p className="mt-1.5 sm:mt-2 text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 font-medium flex items-center gap-1.5 leading-normal">
            {subtitle}
          </p>
        )}
      </div>
