...
// Helper function to calculate total deductions for a homeowner in a specific month
async function calculateDeductions(homeownerId: string, year: number, month: number): Promise<number> {
  try {
    ...
    const result = await dbQuery<{ total_deductions: number }>(query, [homeownerId, lastDay]);
    return Number(result[0]?.total_deductions || 0);
  } catch (error) {
    console.error('Error calculating deductions:', error);
    // If table doesn't exist yet, return 0 (graceful degradation)
    return 0;
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    let { homeowner_id, year, month, amount, status = 'unpaid', official_receipt_number, payment_date, created_by, apply_deductions = true } = body;
    ...
    if (amount === undefined || amount === null || isNaN(Number(amount))) {
      try {
        ...
        baseAmount = settings.length > 0 ? parseFloat(settings[0].setting_value) : 100.00;
      } catch {
        baseAmount = 100.00;
      }
    }
    ...
    if (apply_deductions) {
      totalDeductions = await calculateDeductions(homeowner_id, parseInt(year), parseInt(month));
      finalAmount = Math.max(0, finalAmount - totalDeductions); // Ensure amount doesn't go negative
    }
    ...
    await dbExecute(query, [id, homeowner_id, year, month, finalAmount, status, official_receipt_number || null, payment_date || null, created_by || actor.id]);
    ...
    return NextResponse.json({ success: true, id, activity: activity[0] || null, deductions_applied: totalDeductions });
  } catch (error: any) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { id, status, amount, official_receipt_number, payment_date, apply_deductions = true } = body;
    ...
    let finalAmount = existingDue?.amount;
    let totalDeductions = 0;

    if (amount !== undefined && !isNaN(Number(amount))) {
      let baseAmount = Number(amount);
      
      // Calculate deductions if enabled and we're updating the amount
      if (apply_deductions && hoId && dueYear && dueMonth) {
        totalDeductions = await calculateDeductions(hoId, dueYear, dueMonth);
        finalAmount = Math.max(0, baseAmount - totalDeductions);
      } else {
    ...
    await dbExecute(
      ...
      [
        ...
        JSON.stringify({
          ...
          year: dueYear,
          month: dueMonth,
          base_amount: amount !== undefined ? Number(amount) : existingDue?.amount,
          deductions: totalDeductions,
          final_amount: finalAmount,
          status: finalStatus,
    ...
    return NextResponse.json({ success: true, activity: activity[0] || null, deductions_applied: totalDeductions });
  } catch (error: any) {
