...
export default function SettingsPage() {
  ...
  const [totalHousesAvailable, setTotalHousesAvailable] = useState<string>("100");
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (data.success && data.settings) {
        ...
        if (data.settings.hoa_address) {
          setHoaAddress(data.settings.hoa_address);
        }
        if (data.settings.hoa_tin_number) {
          setHoaTinNumber(data.settings.hoa_tin_number);
        }
        if (data.settings.hoa_registration_number) {
          setHoaRegistrationNumber(data.settings.hoa_registration_number);
        }
        if (data.settings.total_houses_available) {
          setTotalHousesAvailable(data.settings.total_houses_available);
        }
      }
    } catch (err) {
  ...
  // Handle Save HOA Settings
  const handleSaveHoaSettings = async (e: React.FormEvent) => {
    e.preventDefault();
    const totalHouses = parseInt(totalHousesAvailable);
    if (isNaN(totalHouses) || totalHouses < 0) {
      toastError("Validation Error", "Please enter a valid non-negative number for total houses.");
      return;
    }
    ...
    try {
      ...
      const res = await fetch("/api/settings", {
        ...
        body: JSON.stringify({
          hoa_name: hoaName,
          hoa_address: hoaAddress,
          hoa_tin_number: hoaTinNumber,
          hoa_registration_number: hoaRegistrationNumber,
          total_houses_available: totalHouses,
        }),
      });
      ...
      if (data.success) {
        success(
          "HOA Settings Saved",
          `HOA information updated successfully. Total available houses set to ${totalHouses}.`
        );
      } else {
        toastError("Save Failed", data.error || "Could not update HOA settings.");
      }
    } catch (err: any) {
      toastError("Save Failed", err.message || "An error occurred while saving settings.");
    } finally {
      ...
      {activeTab === "hoa" && (
        <div className="max-w-2xl space-y-6">
          <div className="rounded-3xl border border-slate-200/80 dark:border-[#1e2f4d] bg-white dark:bg-[#0e192d] p-6 shadow-subtle space-y-6">
            ...
            <form onSubmit={handleSaveHoaSettings} className="space-y-5 pt-4 border-t border-slate-100 dark:border-[#1e2f4d]">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300 mb-2">
                  HOA Name
                </label>
                <Input
                  ...
                  required
                  className="text-base font-semibold text-slate-900 dark:text-slate-100 rounded-xl"
                />
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5">
                  Official name of your Homeowners Association.
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5">
                  Physical address of the HOA office or headquarters.
                </p>
              </div>
              ...
              <div>
                ...
                <Input
                  type="text"
                  value={hoaTinNumber}
                  onChange={(e) => setHoaTinNumber(e.target.value)}
                  className="text-base font-semibold text-slate-900 dark:text-slate-100 rounded-xl"
                />
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5">
                  Tax Identification Number for official tax purposes.
                </p>
              </div>
              ...
              <div>
                ...
                <Input
                  type="text"
                  value={hoaRegistrationNumber}
                  onChange={(e) => setHoaRegistrationNumber(e.target.value)}
                  className="text-base font-semibold text-slate-900 dark:text-slate-100 rounded-xl"
                />
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5">
                  Official registration number with government agencies.
                </p>
              </div>
              ...
              <div>
                ...
                <Input
                  type="number"
                  min="0"
                  value={totalHousesAvailable}
                  onChange={(e) => setTotalHousesAvailable(e.target.value)}
                  required
                  className="text-base font-semibold text-slate-900 dark:text-slate-100 rounded-xl"
                />
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5">
                  Total number of residential houses available in the community. Used for occupancy calculations.
                </p>
              </div>
              ...
              <Button
                type="submit"
                variant="primary"
                isLoading={isSavingHoa}
                className="gap-2 px-6 py-2.5 rounded-xl font-bold shadow-sm"
              >
                <CheckCircle className="h-4 w-4" />
                <span>Save HOA Settings</span>
              </Button>
            </form>
            ...
            <div className="border-t border-slate-100 dark:border-[#1e2f4d] pt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-[#0c182c] border border-slate-200/70 dark:border-slate-800">
                ...
                <span className="font-semibold text-slate-800 dark:text-slate-200">v1.2.0 (Windows Native LAN)</span>
              </div>
