...
export default function HomeownerDuesDetailPage() {
  ...
  const fetchHomeowner = useCallback(async () => {
    try {
  ...
  const fetchMonthlyDues = useCallback(async () => {
    try {
      setIsLoading(true);
      ...
      // Fetch deductions for this homeowner
      try {
        const dedResponse = await fetch(`/api/homeowners/${homeownerId}/deductions`);
        const dedData = await dedResponse.json();
        if (dedData.success) {
  ...
  useEffect(() => {
    fetchHomeowner();
    fetchMonthlyDues();
  }, [fetchHomeowner, fetchMonthlyDues]);
  ...
  const getDeductionsForMonth = (month: number): number => {
    const lastDay = `${selectedYear}-${String(month).padStart(2, '0')}-31`;
    return deductions
      .filter(d => new Date(d.effective_date) <= new Date(lastDay))
      .reduce((total, d) => total + Number(d.deduction_amount), 0);
  };

  // Get the final amount after applying deductions
  const getFinalAmount = (month: number): number => {
    const baseAmount = getMonthAmount(month);
    const totalDeductions = getDeductionsForMonth(month);
    return Math.max(0, baseAmount - totalDeductions);
  };

  const handleStatusToggle = async (month: number, currentStatus: string) => {
    ...
    if (newStatus === "paid") {
      ...
      const result = await Swal.fire({
        ...
        html: `Are you sure you want to mark dues for <b>${monthName} ${selectedYear}</b> as <b>Unpaid</b>?<br/><span class="text-xs text-slate-500">The previous payment record and OR number will be cleared.</span>`,
        icon: "warning",
  ...
  const processPayment = async (
    ...
    // Use the actual stored amount from the database
    const finalAmount = amountOverride ?? (existingDue?.amount ? Number(existingDue.amount) : getFinalAmount(month));
      ...
      if (existingDue) {
        const response = await fetch("/api/monthly-dues", {
          ...
          body: JSON.stringify({
            id: existingDue.id,
            status: newStatus,
            // Don't send amount if just changing status - preserve the stored amount
        ...
        const response = await fetch("/api/monthly-dues", {
          ...
          body: JSON.stringify({
            ...
            apply_deductions: false, // Don't apply deductions - use the actual amount we calculated
          }),
        });
  ...
  const openEditAmountModal = (month: number) => {
    const currentAmount = getFinalAmount(month);
  ...
  const handleSaveMonthAmount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const finalAmount = numAmount; // Use the entered amount as the final amount
      ...
      if (existingDue) {
        const res = await fetch("/api/monthly-dues", {
          ...
          body: JSON.stringify({
            id: existingDue.id,
            amount: finalAmount,
            apply_deductions: false, // Don't apply deductions when manually setting amount
          }),
        });
        const data = await res.json();
        if (data.success) {
        ...
        const res = await fetch("/api/monthly-dues", {
          ...
          body: JSON.stringify({
            ...
            status: "unpaid",
            created_by: currentUser?.id,
            apply_deductions: false, // Don't apply deductions - use the manually entered amount
          }),
        });
        const data = await res.json();
        if (data.success) {
  ...
  const calculateTotalDue = () => {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      const finalAmount = getFinalAmount(m);
      if (due && due.status === "paid") {
        paidTotal += finalAmount;
      } else {
        unpaidTotal += finalAmount;
      }
    }
      ...
      <div className="rounded-3xl border border-slate-200/80 dark:border-[#1e2f4d] bg-white dark:bg-[#0e192d] p-6 shadow-subtle">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {MONTHS.map((month, index) => {
            ...
            const baseAmount = getMonthAmount(monthNum);
            const finalAmount = getFinalAmount(monthNum);
            const totalDeductions = getDeductionsForMonth(monthNum);
            ...
            return (
              ...
              >
                <div>
                  ...
                  {hasDeductions && (
                    <div className="mb-2 p-2 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50">
                      <div className="text-[10px] font-semibold text-amber-800 dark:text-amber-300">
                        -₱{totalDeductions.toFixed(2)} deductions applied
                      </div>
                    </div>
                  )}
