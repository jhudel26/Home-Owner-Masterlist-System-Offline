...
(async function main() {
  ...
  // ── Step 7: Scripts (db-init.js + db-migrate.js + mysql2) ─────────────────────────────
  step(7, TOTAL, "Bundling db-init script, db-migrate script and mysql2 dependency");
  ...
  // Copy db-migrate.js
  fs.copyFileSync(
    path.join(INSTALLER, "scripts", "db-migrate.js"),
    path.join(DIST_SCRIPTS, "db-migrate.js")
  );
  ...
  step(8, TOTAL, "Copying database schema and migrations");
  ...
  // Copy migrations.sql for existing installations
  fs.copyFileSync(
    path.join(INSTALLER, "database", "migrations.sql"),
    path.join(DIST_DATABASE, "migrations.sql")
  );
  console.log("Schema and migrations copied.");
